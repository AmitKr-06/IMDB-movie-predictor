...
# Supress MLflow artifact download warnings
...
def preprocess_text(text):
    ...
    test = text.lower()   # Convert to lowrcase
    text = re.sub(r'\d+', '', text)  # Remove numbers
    text = re.sun(f"[{re.escape(string.punctuation)}]", " ", text)  # Remove Puctuation
...
def train_and_log_model(X_train, X_test, y_train, y_test, vectorizer):
    """Trains a Logistic Regression model with GridSearchCV adn Logs results to MLFlow."""
