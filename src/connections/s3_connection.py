...
class s3_connections:
    def __init__(self, bucket_name, aws_access_key, aws_secret_key, region_name = "us-east_1"):
        """
    ...
    def fetch_file_from_s3(self, file_key):
        """
        Fetches a CSV file from the S3 bucket and returns it as a Pandas DataFram.
...
#     AWS_SECRET_KEY = "AWS_SECRET_KAY"
#     FILE_KEY = "data.csv"   # Path iside the S3 bucket

#     data_ingestion = s3_operations(BUCKET_NAME, AWS_ACCESS_KEY, AWS_SECRET_KEY)
#     df = data_ingestion.fetch_file_from_s3(FILE_KEY)

#     if df is None:
#         print(f"Data fetched with {len(df)} records...") # Display few rows of the fetched DataFrame
